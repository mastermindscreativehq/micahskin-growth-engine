 ...
 * eventType: one of the ALLOWED_EVENT_TYPES in backend funnelEventService.js
 */
export async function trackFunnelEvent(eventType, { leadId, sessionId, bundleId, value, metadata } = {}) {
  try {
    await fetch(`${BASE_URL}/api/funnel/track`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ eventType, leadId, sessionId, bundleId, value, metadata }),
    })
  } catch {
