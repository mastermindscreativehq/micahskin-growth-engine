...
async function handleIncomingPhoto(message, lead, chatId) {
  ...
  if (currentCount >= MAX_IMAGES) {
    ...
    await sendTelegramToUser(
      chatId,
      `We've already received the maximum ${MAX_IMAGES} photos for your profile.`,
      LEAD_BOT_TOKEN
    ).catch(() => {})
    return
  }
  ...
  if (newCount >= MAX_IMAGES) {
    ...
    await sendTelegramToUser(
      chatId,
      `We’ve received the maximum ${MAX_IMAGES} photos. Thank you!\n\n` +
      `Our skincare team will review your details and prepare your skin assessment shortly 🌿`,
      LEAD_BOT_TOKEN
    ).catch(err => console.error('[ImageUpload] max confirmation send failed:', err.message))
    ...
    await sendTelegramToUser(
      chatId,
      `✅ Photo ${newCount} received. You can send ${remaining} more, or type <b>SKIP</b> to continue.`,
      LEAD_BOT_TOKEN
    ).catch(err => console.error('[ImageUpload] receipt confirmation send failed:', err.message))
  }
}
